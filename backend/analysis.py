...
ATTOM_API_KEY = "2aff15dfc866906a35745c2150f5eb74"
USE_ATTOM = os.getenv("USE_ATTOM", "false").lower() == "true"
...
class PropertyData:
    ...
    last_sale_amount: Optional[float] = None   # NEW
...
class AttomSaleAPI:
    ...
    @staticmethod
    def _parse_latest_sale(payload: Dict) -> Optional[float]:
        try:
            return float(payload["property"][0]["sale"]["saleTrans"]["saleAmt"])
        except (KeyError, IndexError, TypeError, ValueError):
            return None

    @classmethod
    def get_last_sale_amount(cls, address: str) -> Optional[float]:
        url = f"{cls.BASE}/property/snapshot"
        params = {"address": address}
        try:
            r = requests.get(url, headers=cls.HEADERS, params=params, timeout=5)
            r.raise_for_status()
            return cls._parse_latest_sale(r.json())
        except Exception as e:
            logger.warning("Attom lookup failed for %s: %s", address, e)
            return None
    ...
    def _generate_mock_data(self) -> List[Dict]:
        ...
        for i in range(100):
            ...
            property_data = {
                ...
                'assessed_value': assessed_value,
                'last_sale_amount': assessed_value * np.random.uniform(0.7, 1.3)  # mock
            }
...
class DataExtractionAgent:
    ...
    def extract_industrial_properties(self, county: str) -> List[PropertyData]:
        ...
        # Optionally enrich with Attom
        if USE_ATTOM:
            for prop in properties:
                prop["last_sale_amount"] = AttomSaleAPI.get_last_sale_amount(prop["address"])
...
class ComparableDiscoveryAgent:
    ...
    def calculate_similarity_score(self, target: PropertyData, comp: PropertyData) -> float:
        ...
        # Last sale amount ratio
        sale_ratio = 1.0
        if target.last_sale_amount and comp.last_sale_amount and max(target.last_sale_amount, comp.last_sale_amount) > 0:
            sale_ratio = min(target.last_sale_amount, comp.last_sale_amount) / max(target.last_sale_amount, comp.last_sale_amount)

        final_score = (
            area_ratio * self.weight_factors['building_area'] +
            (min(target.lot_size, comp.lot_size) / max(target.lot_size, comp.lot_size)) * self.weight_factors['lot_size'] +
            age_score * self.weight_factors['age'] +
            (1.0 if target.zoning == comp.zoning else 0.0) * self.weight_factors['zoning'] +
            location_score * self.weight_factors['location'] +
            sale_ratio * self.weight_factors['last_sale_amount']
        )
        return final_score
        ...
        return [
            {
                ...
                'last_sale_amount': p.last_sale_amount
            }
...
if __name__ == "__main__":
    ...
    if properties:
        ...
        print(f"Last Sale Amount: ${results['target_property'].get('last_sale_amount', 'N/A')}")
        print(f"Found {len(results['comparables'])} comparables:")
        for i, comp in enumerate(results['comparables'], 1):
            print(f"  {i}. {comp['address']} (Score: {comp['similarity_score']}, {comp['confidence_level']} confidence, Sale: ${comp.get('last_sale_amount', 'N/A')})")
